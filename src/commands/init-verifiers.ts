...
3. **Installed verification packages** (for web apps)
   ...
   - Check MCP configuration (.mcp.json) for browser automation tools:
     ...
     - Yao Chrome Extension MCP (browser-use via Claude's Chrome extension)
...
2. **If NO browser automation tools are detected**, ask if they want to install/configure one:
   ...
   - Options to offer:
     ...
     - **Claude Chrome Extension** - Uses the Yao Chrome extension for browser interaction (requires the extension installed in Chrome)
...
4. **If user chooses Chrome DevTools MCP or Yao Chrome Extension**:
   ...
   - For Yao Chrome Extension, inform them they need the extension installed from the Chrome Web Store
...
**All verifier skills are created in the project root's \`.claude/skills/\` directory.** This ensures they are automatically loaded when Yao runs in the project.
