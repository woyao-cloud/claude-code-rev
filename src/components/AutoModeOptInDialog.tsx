...
export const AUTO_MODE_DESCRIPTION = "Auto mode lets Yao handle permission prompts automatically — Yao checks each tool call for risky actions and prompt injection before executing. Actions Yao identifies as safe are executed, while actions Yao identifies as risky are blocked and Yao may try a different approach. Ideal for long-running tasks. Sessions are slightly more expensive. Yao can make mistakes that allow harmful commands to run, it's recommended to only use in isolated environments. Shift+Tab to change mode.";
type Props = {
  onAccept(): void;
  onDecline(): void;
