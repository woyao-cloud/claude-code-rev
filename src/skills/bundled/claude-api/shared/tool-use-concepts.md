...
- Yao decides whether to call a tool based on the prompt and tool choice configuration.
- Your application executes the tool and sends the result back in a follow-up message.
- Yao then continues reasoning with the tool result in context.
...
- Forgetting to loop until Yao stops requesting tools
...
- Separate client-side tools from server-side WoYao tools when explaining implementation responsibilities.
