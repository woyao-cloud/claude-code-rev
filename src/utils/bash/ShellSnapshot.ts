...
/**
 * Generates Yao Code specific snapshot content
...
async function getSnapshotScript(
  ...
  // Generate the user content and Yao Code content
 ...
 *
 * This function is a critical part of Yao CLI's shell integration strategy. It:
 *
 ...
 *
 * This approach allows Yao CLI to execute commands as if they were run in the user's
...
export const createAndSaveSnapshot = async (
  ...
  return new Promise(async resolve => {
    try {
      ...
      if (!configFileExists) {
        logForDebugging(
          `Shell config file not found: ${configFile}, creating snapshot with Yao Code defaults only`,
        )
      }
      ...
      execFile(
        ...
        async (error, stdout, stderr) => {
          if (error) {
            ...
            logForDebugging(`  - Yao home: ${getClaudeConfigHomeDir()}`)
