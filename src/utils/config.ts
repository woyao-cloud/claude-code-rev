...
export type GlobalConfig = {
  ...
  firstStartTime?: string // ISO timestamp when Yao Code was first started on this machine

  messageIdleNotifThresholdMs: number // How long the user has to have been idle to get a notification that Yao is done generating
  ...
  // Yao Code usage tracking
  claudeCodeFirstTokenDate?: string // ISO timestamp of the user's first Yao Code OAuth token
  ...
  // Yao in Chrome settings
  hasCompletedClaudeInChromeOnboarding?: boolean // Whether Yao in Chrome onboarding has been shown
  claudeInChromeDefaultEnabled?: boolean // Whether Yao in Chrome is enabled by default (undefined means platform default)
  ...
  // Yao Code hint protocol state (<claude-code-hint /> tags from CLIs/SDKs).
...
function saveConfigWithLock<A extends object>(
  ...
  try {
    ...
    if (lockTime > 100) {
      logForDebugging(
        'Lock acquisition took longer than expected - another Yao instance may be running',
      )
...
export function getManagedClaudeRulesDir(): string {
  return join(getManagedFilePath(), '.ycode', 'rules')
}
